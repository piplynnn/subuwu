    ...
    private void ProcessOBDData(string response)
    {
        ...
        if (bytes.Length >= 2)
        {
            ...
            else if (bytes[0] == "41" && bytes[1] == "0D") // Speed
            {
   			...
   			else if (bytes[0] == "41" && bytes[1] == "4A") // Pedal Position D
     		{
    			int pedalRaw = Convert.ToInt32(bytes[2], 16);
    			throttleper = (pedalRaw * 100f) / 255f;
    			Debug.Log("🦶 Pedal Position D = " + throttleper + "%");
			}
